...
import { Edit, Eye, Loader2, Plus, Search, Trash2, Upload, ArrowRight, Film, ImageIcon, ChevronLeft, ChevronRight, TrendingUp } from "lucide-react";
...
const initialFormData = {
  ...
  posterId: null,
  teaserId: null,
  isTrending: false, // Giá trị mặc định ban đầu
};

type ModalMode = "CREATE" | "VIEW" | "EDIT";

export function MovieManagementTab() {
                      ...
                      <td className="py-4 px-4">
                        <div className="flex flex-col gap-1">
                          <p className="font-semibold flex items-center gap-1.5">
                            {movie.title}
                            {/* THÊM BADGE TRENDING Ở LIST BẢNG */}
                            {movie.isTrending && (
                              <Badge className="bg-amber-500/20 text-amber-600 border-amber-500/30 text-[10px] h-4 px-1.5 py-0 flex items-center gap-0.5 font-medium">
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="bg-white border-gray-200 max-w-3xl text-gray-900 overflow-y-auto max-h-[90vh]">
          ...
          <form className="space-y-4 mt-4 text-gray-900" onSubmit={handleSubmitForm}>
            ...
            {/* THÊM KHU VỰC IS TRENDING TRONG DIALOG FORM */}
            <div className="flex items-center space-x-3 bg-amber-50/50 border border-amber-200/60 rounded-xl p-3">
              <input
                id="isTrending"
                type="checkbox"
                disabled={modalMode === "VIEW"}
                checked={formData.isTrending}
                onChange={(event) => setFormData((current) => ({ ...current, isTrending: event.target.checked }))}
                className="w-4 h-4 text-amber-600 border-gray-300 rounded focus:ring-amber-500 accent-amber-500 disabled:opacity-60 cursor-pointer"
              />
              <div className="grid gap-0.5 leading-none">
                <label
                  htmlFor="isTrending"
                  className="text-sm font-semibold text-gray-900 flex items-center gap-1 cursor-pointer select-none"
                >
                  <TrendingUp className="w-4 h-4 text-amber-600" /> Phim thịnh hành
                </label>
              </div>
            </div>

            {/* PHẦN UPLOAD VÀ HIỂN THỊ MEDIA */}
            <div className="space-y-6 border-t border-gray-200 pt-4">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-7 items-center gap-2 bg-gray-50 p-3 rounded-xl border border-gray-100">
                  <div className="col-span-3 flex flex-col items-center justify-center border border-dashed border-gray-200 rounded-lg bg-white h-32 text-center overflow-hidden">
                    {oldUrls.posterUrl ? (
                      <img src={oldUrls.posterUrl} alt="Old Poster" className="w-full h-full object-cover" />
                    ) : (
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-7 items-center gap-2 bg-gray-50 p-3 rounded-xl border border-gray-100">
                  <div className="col-span-3 flex flex-col items-center justify-center border border-dashed border-gray-200 rounded-lg bg-white h-32 text-center overflow-hidden">
                    {oldUrls.teaserUrl ? (
                      <video src={oldUrls.teaserUrl} className="w-full h-full object-cover bg-black" controls muted />
                    ) : (
                      <div className="text-xs text-gray-500 flex flex-col items-center p-2">
